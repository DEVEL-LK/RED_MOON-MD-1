...
try{
// ⚠️ මෙහි ප්‍රතිචාර පණිවිඩය වෙනස් කරන ලදී.
if (!q || !q.includes('https://cinesubz.net/movies/')) {
    console.log('Invalid input:', q);
    // ".tv" වෙනුවට ".cinetv" හෝ ".ctv" යන ඔබේ TV show command එක යොදමු.
    return await reply('*❗ This is a TV series, please use .cinetv command.*');
}
...
if (!q || !q.includes('https://cinesubz.net/tvshows')) {
    console.log('Invalid input:', q);
    // ".mv" වෙනුවට ".cine" හෝ ".cinedl" යන ඔබේ Movie command එක යොදමු.
    return await reply('*❗ This is a movie, please use .cine command.*');
}

let sadas = await cinesubz_tvshow_info(q)
let msg = `*☘️ 𝗧ɪᴛʟᴇ ➮* *_${sadas.data.title || 'N/A'}_*

*📅 𝗥ᴇ𝘭ᴇꜱᴇᴅ ᴅᴀᴛᴇ ➮* _${sadas.data.date || 'N/A'}_
*🌎 𝗖ᴏᴜɴᴛʀʏ ➮* _${sadas.data.country || 'N/A'}_
*💃 𝗥ᴀᴛɪɴɢ ➮* _${sadas.data.imdb || 'N/A'}_
*⏰ 𝗥ᴜɴᴛɪᴍᴇ ➮* _${sadas.data.runtime || 'N/A'}_
